angular.module('market', ['ngStorage']).controller('indexController', function ($scope, $http, $localStorage) {
    ...
    $scope.fillTable = function () {
        ...
        $http.get('http://localhost:8081/market/api/v1/cart')
            .then(function (response) {
                //$scope.products = response.data;
    ...
    $scope.addProductAmount=function (id){
        $http.get('http://localhost:8081/market/api/v1/cart/add/'+id)
            .then(function (response){
                $scope.fillTable();
            });
    };
    ...
    $scope.subProductAmount=function (id){
        $http.get('http://localhost:8081/market/api/v1/cart/sub/'+id)
            .then(function (response){
                $scope.fillTable();
            });
    };
    ...
    $scope.removeItem=function (id){
        $http.delete('http://localhost:8081/market/api/v1/cart/remove/'+id)
            .then(function (response){
                $scope.fillTable();
            });
    ...
    $scope.createOrder=function(){
        $http.post('http://localhost:8080/api/v1/orders/create')
            .then(function (response) {
                $scope.fillTable();
            });
    }
